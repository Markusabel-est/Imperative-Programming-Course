...
TEST(enter_command, hyphen_maddness)
{
    ...
    int no_of_words = enter_command(HYPHEN_MADDNESS_FILENAME, content);
    EXPECT_EQ(no_of_words, NO_OF_WORDS_HYPHEN_MADDNESS);
...
{
    string content[MAX_NO_OF_WORDS];
    int no_of_words = enter_command(DESKTOP_FILENAME, content);
    EXPECT_EQ(no_of_words, NO_OF_WORDS_DESKTOP);
...
{
    string content[MAX_NO_OF_WORDS];
    int no_of_words = enter_command(VOGON_POEM_FILENAME, content);
    EXPECT_EQ(no_of_words, NO_OF_WORDS_VOGON_POEM);
...
TEST(enter_command, alice)
{
    ...
    EXPECT_GT(no_of_words, 0);  // Ensure a positive number of words was read
}
