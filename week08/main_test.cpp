...
{
    Length actual;
    stringstream ss("6:00");
    ss >> actual;
    EXPECT_EQ(new_length (6,0), actual);
}
TEST(operators, length_input1)
{
    Length actual;
    stringstream ss("1:01");
    ss >> actual;
    EXPECT_EQ(new_length (1,1), actual);
}

TEST(operators, length_input2)
{
    Length actual;
    stringstream ss("16:59");
    ss >> actual;
    EXPECT_EQ(new_length (16,59), actual);
}

TEST(operators, length_input3)
{
    Length actual;
    stringstream ss("0:20");
    ss >> actual;
    EXPECT_EQ(new_length (0,20), actual);
}

TEST(operators, length_output)
{
    stringstream actual;
    actual << new_length (10,13);
    EXPECT_EQ("10:13", actual.str());
}

TEST(operators, length_output1)
{
    stringstream actual;
    actual << new_length (1,1);
    EXPECT_EQ("1:01", actual.str());
    
}

TEST(operators, length_output2)
{
    stringstream actual;
    actual << new_length (6,0);
    EXPECT_EQ("6:00", actual.str());
   
}

TEST(operators, length_output3)
{
    stringstream actual;
    actual << new_length (0,20);
    EXPECT_EQ("0:20", actual.str());
   
}

TEST(operators, length_add)
{
    // Test your implementation of + for all combinations of the following
    // Length values
    const Length L1 = {42,55}, L2 = {3,4}, L3 = {24,6};
}

TEST(operators, length_add1)
{
    Length length_exp = {45, 59};
    const Length L1 = {42,55}, L2 = {3,4};
    Length sum = L1 + L2;
    EXPECT_EQ(sum, length_exp);

}

TEST(operators, length_add2)
{
    Length length_exp = {27, 10};
    const Length L3 = {24,6}, L2 = {3,4};
    Length sum = L3 + L2;
    EXPECT_EQ(sum, length_exp);

}

TEST(operators, length_add3)
{
    Length length_exp = {67, 1};
    const Length L1 = {42,55}, L3 = {24,6};
    Length sum = L1 + L3;
    EXPECT_EQ(sum, length_exp);
